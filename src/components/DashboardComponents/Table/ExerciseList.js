...
export const ExerciseList = ({ exercises, setExercises, pagesVisited, exercisesPerPage, info, setInfo, setInfoArr }) => {
  ...
  ///*** REMOVE COMMENTED OUT FUNCTION WHEN NOT USING MOCK DATA TO ALLOW DELETE - CHANGE ONCLICK FUNCTION TO onClick={() => { deleteExercise(ex._id) }}***///
          ...
          return (
            <tr key={index} className='bg-secondary'>
              ...
              <td className='py-4 lg:py-2'>
                <Link to={"/edit/" + ex._id}>Edit</Link> | <button onClick={deleteExericseMock}><FaTrash /></button> | <button className='align-text-top' onClick={() => handleInfoClick(ex._id)}><FaInfoCircle /></button>
              </td>
            </tr>
          )
        })}
